...
export const PatternBuilder: React.FC<Props> = ({
  patternId,
  customIndexes,
  onChangePatternId,
  onChangeCustom,
  ...
  return (
    <div className="w-full max-w-xs mb-4">
      <div className="grid grid-cols-5 gap-2 mb-2">
        {headers.map((h) => (
          <div key={h} className="flex items-center justify-center font-bold">
            {h}
          </div>
        ))}
      ...
      <select
        className="mt-4 h-12 w-full rounded-md border border-gray-300 px-3 py-2"
      ...
      <button
        type="button"
        className="mt-4 h-12 w-full rounded-md border border-gray-300 py-2 font-semibold"
        onClick={() => {
          onChangePatternId("custom");
          onChangeCustom([]);
        }}
